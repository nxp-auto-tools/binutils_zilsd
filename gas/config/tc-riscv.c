...
static struct riscv_combiner *insn_combiner[256];
...
// addibne 
static int
riscv_xl_pack_addibne_scale (offsetT imm)
{
  switch (imm)
    {
    ...
    switch (*++oparg)
      {
        ...
        case'L':
		      switch (*++oparg)
          {
            case 'b': used_bits |= ENCODE_XLCZ_LB_IMM (-1U); break;
            case 'h': used_bits |= ENCODE_XLCZ_LH_IMM (-1U); break;
            case 'w': used_bits |= ENCODE_XLCZ_LW_IMM (-1U); break;
            case 'd': used_bits |= ENCODE_XLCZ_LD_IMM (-1U); break;
          default:
            goto unknown_validate_operand;
          }
          break;
        case'O': used_bits |= ENCODE_XLCZ_BMRK_IMM(-1U); break;
        ...
        case'S':
		      switch (*++oparg)
          {
            case 'b': used_bits |= ENCODE_XLCZ_SB_IMM (-1U); break;
            case 'h': used_bits |= ENCODE_XLCZ_SH_IMM (-1U); break;
            case 'w': used_bits |= ENCODE_XLCZ_SW_IMM (-1U); break;
            case 'd': used_bits |= ENCODE_XLCZ_SD_IMM (-1U); break;
          default:
            goto unknown_validate_operand;
          }
          break;
        case'W': used_bits |= ENCODE_XLCZ_LGPW_IMM(-1U); break;
        case'w': used_bits |= ENCODE_XLCZ_SGPW_IMM(-1U); break;
        default:
          goto unknown_validate_operand;
      }
    break;
	     ...
	     bfd_reloc_code_real_type reloc_type)
{
  ...
  if (reloc_type != BFD_RELOC_UNUSED)
    ...
    || reloc_type == BFD_RELOC_RISCV_XL_ADDIBNE
...
cache_an_insn (struct riscv_cl_insn *insn,
		    expressionS *imm_expr,
		    bfd_reloc_code_real_type reloc_type, int index)
{
  memcpy((void*)&(insn_combiner[index]->imm_expr),
      (void*)imm_expr, sizeof(expressionS));
  memcpy((void*)&(insn_combiner[index]->insn), (void*)insn,
...
riscv_xlcz_addibne_1 (struct riscv_cl_insn *insn, expressionS *imm_expr,
	const bfd_reloc_code_real_type reloc_type)
{
  int rd, rs1, scale;
  ...
  scale = riscv_xl_pack_addibne_scale(imm_expr->X_add_number);
...
static bfd_boolean
riscv_xlcz_addibne_2 (struct riscv_cl_insn *insn,
...
static bfd_boolean
riscv_xlcz_addibne_out (struct riscv_cl_insn *insn, expressionS *imm_expr,
	const bfd_reloc_code_real_type imm_reloc)
{
  offsetT label_loc, insn_loc, offset;
  int rd = EXTRACT_OPERAND (RD, insn_combiner[0]->insn.insn_opcode);
  int scale = riscv_xl_pack_addibne_scale (
  ...
  if (imm_expr->X_op == O_constant
      ...
      && VALID_XL_ADDIBNE_IMM (1*imm_expr->X_add_number))
    {
      insn->insn_opcode = MATCH_XL_ADDIBNE;
  ...
  */
  //if (riscv_subset_supports (&riscv_rps_as, "xxlcz")
  if (imm_expr->X_op != O_constant
      && S_IS_DEFINED (imm_expr->X_add_symbol))
    {
      label_loc = S_GET_VALUE (imm_expr->X_add_symbol);
      ...
      offset = insn_loc - label_loc;
          {
            insn->insn_opcode = MATCH_XL_ADDIBNE;
            INSERT_OPERAND (XL_SCALE, *insn, scale);
            INSERT_OPERAND (RD, *insn, rd_addi);
            INSERT_OPERAND (RS1, *insn, xxc_rs);   
            return TRUE;
          }
    }
  	insn->insn_opcode = MATCH_XL_ADDIBNE;
...
init_insn_combiner (void)
{
    if (!insn_combiner[0])
    {
      ...
      {
        insn_combiner[ii] = (struct riscv_combiner *)
	      xmalloc (sizeof (struct riscv_combiner));
        insn_combiner[ii]->idx = 0;
        insn_combiner[ii]->matcher = riscv_comb_matchers;
      }
    }
}
	  ...
	  bfd_reloc_code_real_type *imm_reloc, htab_t hash)
{
  ...
  for ( ; insn && insn->name && strcmp (insn->name, str) == 0; insn++)
      ...
      case 'b':
        if (oparg[1]=='B') {
          ...
          *imm_reloc = BFD_RELOC_RISCV_XL_BMRK;
          ...
          asarg = expr_end;
          //if (imm_expr->X_add_number<(-128) || imm_expr->X_add_number>127) break;
          ip->insn_opcode |= ENCODE_XLCZ_DECBNEZ_SCALE (imm_expr->X_add_number);
          ++oparg;  
        } else if (oparg[1]=='j') {
          my_getExpression (imm_expr, asarg);
          check_absolute_expr (ip, imm_expr, FALSE);
        ...
        /* nuclei addibne */
          ...
          switch (*++oparg)
          {
            case 'b':
              ...
              ip->insn_opcode |= ENCODE_XLCZ_LB_IMM (imm_expr->X_add_number);
              continue;
            case 'h':
              my_getExpression (imm_expr, asarg);
              check_absolute_expr (ip, imm_expr, FALSE);
              asarg = expr_end;
              // if (imm_expr->X_add_number<-16 || imm_expr->X_add_number>15) break;
              ip->insn_opcode |= ENCODE_XLCZ_LH_IMM (imm_expr->X_add_number);
              continue;
            case 'w':
              my_getExpression (imm_expr, asarg);
              check_absolute_expr (ip, imm_expr, FALSE);
              asarg = expr_end;
              // if (imm_expr->X_add_number<-16 || imm_expr->X_add_number>15) break;
              ip->insn_opcode |= ENCODE_XLCZ_LW_IMM (imm_expr->X_add_number);
              continue;
            case 'd':
              my_getExpression (imm_expr, asarg);
              check_absolute_expr (ip, imm_expr, FALSE);
              asarg = expr_end;
              // if (imm_expr->X_add_number<-16 || imm_expr->X_add_number>15) break;
              ip->insn_opcode |= ENCODE_XLCZ_LD_IMM (imm_expr->X_add_number);
              continue;
          default:
            goto unknown_riscv_ip_operand;
          }
          ...
          switch (*++oparg)
          {
            case 'b':
              ...
              asarg = expr_end;
              // if (imm_expr->X_add_number<-16 || imm_expr->X_add_number>15) break;
              ip->insn_opcode |= ENCODE_XLCZ_SB_IMM (imm_expr->X_add_number);
              continue;
            case 'h':
              my_getExpression (imm_expr, asarg);
              check_absolute_expr (ip, imm_expr, FALSE);
              asarg = expr_end;
              // if (imm_expr->X_add_number<-16 || imm_expr->X_add_number>15) break;
              ip->insn_opcode |= ENCODE_XLCZ_SH_IMM (imm_expr->X_add_number);
              continue;
            case 'w':
              my_getExpression (imm_expr, asarg);
              check_absolute_expr (ip, imm_expr, FALSE);
              asarg = expr_end;
              // if (imm_expr->X_add_number<-16 || imm_expr->X_add_number>15) break;
              ip->insn_opcode |= ENCODE_XLCZ_SW_IMM (imm_expr->X_add_number);
              continue;
            case 'd':
              my_getExpression (imm_expr, asarg);
              check_absolute_expr (ip, imm_expr, FALSE);
              asarg = expr_end;
              // if (imm_expr->X_add_number<-16 || imm_expr->X_add_number>15) break;
              ip->insn_opcode |= ENCODE_XLCZ_SD_IMM (imm_expr->X_add_number);
              continue;
          default:
            goto unknown_riscv_ip_operand;
          }      
          ...
          asarg = expr_end;
          //if (imm_expr->X_add_number<0 || imm_expr->X_add_number>31) break;
          ip->insn_opcode |= ENCODE_XLCZ_SGPW_IMM (imm_expr->X_add_number);
          ++oparg;          
        } else {
          my_getExpression (imm_expr, asarg);
          asarg = expr_end;
    ...
    //  illegal_operand:
	  //     as_fatal (_("internal: illegal operand `%s'"),
		// 	opargStart);
	    }
	  break;
	}
     ...
     if (matchers[idx].check_2 (insn, imm_expr, imm_reloc))
	    {
	      ...
	      release_cached_insn (1);
        imm_reloc = BFD_RELOC_RISCV_XL_ADDIBNE;
	      append_insn (insn, imm_expr, imm_reloc);
        insn_combiner[0]->idx = 0;
        md_flag_idx = 0;
	      return;
	    }
      md_flag_idx = md_flag_idx + 1;
      cache_an_insn (insn, imm_expr, imm_reloc, md_flag_idx);
      return;
	}

      gas_assert (insn_combiner[0]->idx == 0);
      // 查找matchers列表
...
md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_RISCV_XL_ADDIBNE:
      if (fixP->fx_addsy)
	{
	  /* Fill in a tentative value to improve objdump readability.  */
	  bfd_vma target = S_GET_VALUE (fixP->fx_addsy) + *valP;
	  bfd_vma delta = target - md_pcrel_from (fixP);
	  bfd_putl32 (bfd_getl32 (buf) | (ENCODE_XL_ADDIBNE_IMM (-delta)), buf);
	}
      break;
...
riscv_cleanup (void)
{
  ...
  if (use_insn_combiner ()
      ...
      && (insn_combiner[0]->idx))
    {
      for(int ii = 0; ii <= md_flag_idx; ii++)
      {
        append_insn (&(insn_combiner[ii]->insn), &(insn_combiner[ii]->imm_expr), insn_combiner[ii]->imm_reloc);
        insn_combiner[ii]->idx = 0;
      }
    }
    md_flag_idx = 0;
...
md_convert_frag_branch (fragS *fragp)
{
  ...
  if (RELAX_BRANCH_RVC (fragp->fr_subtype))
      ...
      switch (RELAX_BRANCH_LENGTH (fragp->fr_subtype))
	{
      ...
      else if ((insn & MASK_XL_ADDIBNE) == MASK_XL_ADDIBNE)
	      {
	        insn = MATCH_XL_ADDIBNE
		    | (rs1 << OP_SH_RD)
		    | EXTRACT_OPERAND (XL_SCALE, insn) << OP_SH_XL_SCALE;
	        exp.X_add_number *= -1;
	      }
	    else
	      abort ();
      ...
      if ((insn & MASK_XL_ADDIBNE) == MATCH_XL_ADDIBNE)
        {
          reloc = BFD_RELOC_RISCV_XL_ADDIBNE;
        }
	    else
  ...
  switch (RELAX_BRANCH_LENGTH (fragp->fr_subtype))
    ...
    case 8:
      ...
      /* Invert the branch condition.  Branch over the jump.  */
      insn = bfd_getl32 (buf);
      if ((insn & MATCH_XL_ADDIBNE) != MATCH_XL_ADDIBNE)
      {
    ...
    case 4:
      ...
      else if(((insn & MATCH_XL_ADDRCHK) == MATCH_XL_ADDRCHK) 
...
riscv_write_out_attrs (void)
{
  ...
  //as_warn("riscv_write_out_attrs4\n");
}
